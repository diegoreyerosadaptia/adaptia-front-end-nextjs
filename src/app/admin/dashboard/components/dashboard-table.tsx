    ...
    <Card className="border-gray-200/20 shadow-lg">
      <CardHeader className="border-b border-gray-200/20 bg-white">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            <CardTitle className="text-2xl font-heading text-[#163F6A]">Organizaciones y Análisis</CardTitle>
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-gray-50 border-b border-gray-200/20">
              <tr>
                <th className="px-3 py-3 text-left text-xs font-semibold text-[#163F6A]">Organización</th>
                <th className="px-3 py-3 text-left text-xs font-semibold text-[#163F6A]">Industria</th>
                <th className="px-3 py-3 text-left text-xs font-semibold text-[#163F6A]">País</th>
                <th className="px-3 py-3 text-left text-xs font-semibold text-[#163F6A]">Empleados</th>
                <th className="px-3 py-3 text-center text-xs font-semibold text-[#163F6A]">Estado Análisis</th>
                <th className="px-3 py-3 text-center text-xs font-semibold text-[#163F6A]">Estado Pago</th>
                <th className="px-3 py-3 text-center text-xs font-semibold text-[#163F6A]">Acciones</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-200/20">
              {filteredOrgs && filteredOrgs.length > 0 ? (
                filteredOrgs.map((org, index) => {
                  ...
                  const completedCount = org.analysis?.filter((a: any) => a.status === "COMPLETED").length || 0
                  const pendingCount = org.analysis?.filter((a: any) => a.status === "PENDING").length || 0
                  const failedCount = org.analysis?.filter((a: any) => a.status === "FAILED").length || 0
                    ...
                    >
                      ...
                      <td className="px-3 py-3">
                        <div className="flex flex-wrap gap-1 justify-center">
                          ...
                          {analysisCount === 0 && (
                            <span className="text-[10px] text-gray-600 italic">Sin análisis</span>
                          )}
                        </div>
                      </td>
